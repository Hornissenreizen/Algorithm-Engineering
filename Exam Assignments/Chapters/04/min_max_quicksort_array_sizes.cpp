...
int main() {
    std::ofstream file("sorting_algorithms_running_times_array.csv");
    std::ofstream speedup("speedup_array.csv");
    ...
    file << "Array Size,std::sort,min_max_quicksort,__gnu_parallel::sort\n";
    speedup << "Array Size,min_max_quicksort,__gnu_parallel::sort\n";
    for (size_t size = 1<<17; size <= 1ul<<28; size <<= 1) {
        ...
        std::vector<int64_t> data_copy_parallel = data;  // duplicate data for __gnu_parallel:

        omp_set_num_threads(omp_get_num_procs());
        file << (size >> (20-3)) << ','; // print size in MB (int64_t = 2^3 bytes)
        speedup << (size >> (20-3)) << ',';
        ...
        file << time_min_max_quicksort << ',';
        speedup << time_std_sort / time_min_max_quicksort << ',';
        ...
        file << time_gnu_parallel << '\n';
        speedup << time_std_sort / time_gnu_parallel << '\n';
    }
    file.close();
    speedup.close();
    return 0;
