...
int main() {
    std::ofstream file("sorting_algorithms_running_times.csv");
    std::ofstream speedup("speedup.csv");
    ...
    file << "threads,std::sort,min_max_quicksort,__gnu_parallel::sort\n";
    speedup << "threads,min_max_quicksort,__gnu_parallel::sort\n";
    for (size_t num_threads = 1; num_threads <= omp_get_num_procs(); num_threads++) {
        for (int64_t i = 0; i < SIZE; ++i) {
